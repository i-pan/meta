...
ma.id.mb.BAD <- c(8442) 
# reasons
# 8442 - tau2eb did not converge, do separately
ma.id.mbin <- sort(unique(many.studies.bin[, "ma_id"]))
ma.id.mbin <- ma.id.mbin[!(ma.id.mbin %in% ma.id.mb.BAD)]

ma.id.lcont <- sort(unique(large.studies.cont[, "ma_id"]))

ma.id.mc.BAD <- c(170, 709, 719, 720, 721, 722, 723, 2503)
# reasons
# 170 - bad data? something to do with ECDF being NA/unique
# 709 - tau2eb did not converge, do separately
# 719 - tau2eb did not converge, do separately
# 720 - tau2eb did not converge, do separately
# 721 - tau2eb
# 722 - tau2eb
# 723 - tau2eb
# **note 1503 needs high gmo.xgrid
# 1580 - mle tau2 doesn't converge
# 1581 - mle tau2 doesn't converge
# 2503 - the data sucks 
# 3911 - tau2eb
# 3913 - tau2eb
...
run.gmeta <- function(ma.set, ma.type, data.type) {
	...
	if(ma.type == "large" & data.type == "bin") {
		...
		for(each.ma in ma.id.lbin) {
			...
			print(paste("Starting:", "ma_id", each.ma, ma.type, data.type, counter, "of", length(ma.id.lbin)))
			...
			gmeta.MHOR <- gmeta(gmi=ma.2by2, gmi.type='2x2', method='MH', gmo.xgrid=gmo.Xgrid)
	...
	if(ma.type == "many" & data.type == "bin") {
		...
		for(each.ma in ma.id.mbin) {
			print(paste("Starting:", "ma_id", each.ma, ma.type, data.type, counter, "of", length(ma.id.mbin)))
			# grab individual one m-a at a time
	...
	if(ma.type == "large" & data.type == "cont") {
		...
		for(each.ma in ma.id.lcont) {
			print(paste("Starting:", "ma_id", each.ma, ma.type, data.type, counter, "of", length(ma.id.lcont)))
			# grab individual one m-a at a time
	...
	if(ma.type == "many" & data.type == "cont") {
		...
		# run gmeta
		#####
		gmo.Xgrid <- seq(-10, 10, by=0.001)
		counter <- 1
		for(each.ma in ma.id.mcont) {
			print(paste("Starting:", "ma_id", each.ma, ma.type, data.type, counter, "of", length(ma.id.mcont)))
			# grab individual one m-a at a time
